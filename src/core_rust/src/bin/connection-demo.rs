...
use neurograph_core::{active_levels, connection_flags, Connection, ConnectionType};
...
fn main() {
    ...
    println!(
        "   Rigidity: {:.2} (how firm the connection is)",
        conn.get_rigidity()
    );
    println!(
        "   Pull strength: {:.2} (attraction force)",
        conn.pull_strength
    );
    println!(
        "   Preferred distance: {:.2} (desired spacing)\n",
    ...
    println!(
        "   Active on L5 (Cognitive): {}",
        conn.is_level_active(active_levels::L5_COGNITIVE)
    );
    println!(
        "   Active on L8 (Abstract): {}",
        conn.is_level_active(active_levels::L8_ABSTRACT)
    );
    println!(
        "   Active on L1 (Physical): {}\n",
    ...
    println!(
        "   Bidirectional: {}",
        conn.has_flag(connection_flags::BIDIRECTIONAL)
    );
    println!(
        "   Persistent: {}",
        conn.has_flag(connection_flags::PERSISTENT)
    );
    println!(
        "   Reinforced: {}\n",
    ...
    println!(
        "   - Tokens: {} → {}",
        conn_copy.token_a_id, conn_copy.token_b_id
    );
    ...
    for (i, (conn_type, a, b, desc)) in examples.iter().enumerate() {
        ...
        println!(
            "      {} → {} (type: {})",
            c.token_a_id, c.token_b_id, c.connection_type
        );
    }
    ...
    println!(
        "   - Pull strength: {:.2} (positive = attract)",
        attraction.pull_strength
    );
    println!(
        "   - Preferred distance: {:.2} (very close)",
        attraction.preferred_distance
    );
    ...
    println!("\n   Antonym (repulsion):");
    println!(
        "   - Pull strength: {:.2} (negative = repel)",
        repulsion.pull_strength
    );
    println!(
        "   - Preferred distance: {:.2} (far apart)",
        repulsion.preferred_distance
    );
    ...
    println!(
        "   Time since last activation: {} seconds",
        test_conn.time_since_activation()
    );
