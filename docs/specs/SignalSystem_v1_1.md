...
### Фаза 1: Core Structures (2-3 дня)
- [ ] SignalEvent struct (256 bytes)
- [ ] EventTypeRegistry
- [ ] ProcessingResult
- [ ] Unit тесты

### Фаза 2: Filter System (2 дня)
- [ ] SubscriptionFilter
- [ ] FilterCondition variants
- [ ] Wildcard matching
- [ ] Compile from JSON

### Фаза 3: SignalSystem Core (3-4 дня)
- [ ] emit() implementation
- [ ] subscribe()/unsubscribe()
- [ ] notify_subscribers()
- [ ] Integration with Grid/Graph

### Фаза 4: Python Bindings (2-3 дня)
- [ ] PySignalSystem class
- [ ] Dict↔Struct conversion
- [ ] GIL release optimization
- [ ] Callback mechanism

### Фаза 5: Integration & Testing (2 дня)
- [ ] Integration tests
...
v1.1 полностью совместима с v1.0:
- Все существующие API Graph (spreading activation) работают без изменений
- Новые методы добавляются, не заменяют существующие
- Guardian event system продолжает работать параллельно

### Интеграция

```rust
// v1.0 способ (продолжает работать)
guardian.emit_event(EventType::TokenCreated, MODULE_GRID, token_id, 0);

// v1.1 способ (новый)
signal_system.emit(SignalEvent::new(event_type_id, vector));
```
